...
SEC_IN_DAY = 86400
client = EurostatAPIClient("v2.2", "json", "en") # EuroStat API
...
def get_gdp_region(country):
    country = countries[country]
    params = {"unit": "MIO_EUR", "geo": nuts_codes[country], "lastTimePeriod": 1}
    df = client.get_dataset("nama_10r_2gdp", params).to_dataframe()

    # Drop NAs
    df.dropna(inplace=True)
    df["region_name"] = df["geo"].apply(lambda x: nuts_codes[country][x])
    df["values"] = df["values"] / 1000
    df["time"] = pd.to_datetime(df["time"])
...
def get_gdp_capita_region(country):
    country = countries[country]
    params = {"unit": "EUR_HAB", "geo": nuts_codes[country], "lastTimePeriod": 1}
    df = client.get_dataset("nama_10r_2gdp", params).to_dataframe()
...
def get_govt_debt(country):
    country = countries[country]
    params = {"geo": country, "unit": "PC_GDP", "sector": "S13", "na_item": "GD"}
...
def get_govt_budget(country):
    country = countries[country]
    params = {"geo": country, "unit": "PC_GDP", "sector": "S13", "na_item": "B9"}
    df = client.get_dataset("gov_10dd_edpt1", params).to_dataframe()
...
def get_poverty_risk_region(country):
    country = countries[country]
    params = {"geo": nuts_codes[country], "lastTimePeriod": 1}
...
def get_life_expectancy_region(country):
    country = countries[country]
    params = {"sex": "T", "geo": nuts_codes[country], "lastTimePeriod": 1}
...
def get_doctors(country):
    country = countries[country]
    params = {"geo": country, "unit": "P_HTHAB", "wstatus": "PRACT", "isco08": "OC221"}
...
def get_hospital_beds(country):
    country = countries[country]
    params = {"geo": country, "unit": "P_HTHAB", "facility": "HBEDT"}
...
def get_early_leavers(country):
    country = countries[country]
    params = {"geo": country, "sex": "T", "unit": "PC", "age": "Y18-24"}
...
def get_employment_graduates(country):
    country = countries[country]
    params = {"geo": country, "sex": "T", "unit": "PC", "age": "Y20-34"}
...
def get_ghg_emissions(country):
    country = countries[country]
    params = {"geo": country, "src_crf": "TOTX4_MEMONIA", "unit": "T_HAB"}
...
def get_new_cases(country):
    # Read OurWorldinData COVID data from official GitHub repo
    df = pd.read_csv(
...
def get_vaccinations(country):
    ...
    df_vaccinations = pd.read_csv(
        "https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/vaccinations/country_data/{}.csv".format(
            country
        )
    )
